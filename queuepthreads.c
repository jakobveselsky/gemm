...
#define THREADS 1 //defines how many threads will be made
...
int main(int argc, char *argv[]){
	...
	int count; //creates variable to check if matrix vector multiplaction is done on all threads
	int N;
	...
	do{
		count = 0;
		for(int i = 0; i < THREADS; i++){
			count += t_info[i].done_multiplying;
        	}
	}while(count !=0);	
